...
top.title('Foot Ulcer Detection')
img= PhotoImage(file='bg (9).png', master=top)
img_label= Label(top,image=img)
img_label.place(x=0, y=0)

#top.configure(bg='#F0F0F0')  # Light grayish for a clean, mild look
...
def classify():
    ...
    if(pclass=="foot"):
        ...
        if len(boxes) > 0:
            ...
            uploaded.thumbnail(((top.winfo_width() / 2), (top.winfo_height() / 2)))  # Adjust thumbnail size for better view
...
# Instruction label
label = Label(top, font=('Helvetica', 12), bg='#F0F0F0', fg='#555555')  # Light gray background with soft dark text
...
upload.configure(background='#5E81AC', foreground='white', font=('Helvetica', 14, 'bold'), relief="flat", borderwidth=0)  # Same color for consistency
...
classify_button.configure(background='#5E81AC', foreground='white', font=('Helvetica', 14, 'bold'), relief="flat", borderwidth=0)  # Same color for consistency
classify_button.grid(row=7, column=1, padx=20, pady=10)  # Second button, side by side in row 7

# Heading with clean, large font
heading = Label(top, text="Foot Ulcer Detection", pady=20, font=('Helvetica', 30, 'bold'))
